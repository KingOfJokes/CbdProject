...
#讀入指數價格資料
Indexprice = pd.read_excel('./Index_mktc5.xlsx')
#Indexprice
...
for index,value in collections.iterrows():
    if (start_date <= value["post_time"].to_pydatetime()) and (end_date >=value["post_time"].to_pydatetime()):
        if ("蘋概" in (str(value["content"])+str(value["title"])))  or ("台積電" in (str(value["content"])+str(value["title"])))  or ("鴻海" in (str(value["content"])+str(value["title"]))):
...
def main(day,multi,ver,chithresh):
    #day = 5
    #multi = 1
    #length = 850
    ...
    def remove_stopwords(content):
        ...
        content = re.sub(r'[Ａ-Ｚａ-ｚ０-９]','',content)  #移除英文&數字
        '''
        for s in stopword:
            print(s)
            re.sub(s,'',content)
    ...
    for key in increase_keyword_dict.keys():
        ...
        for art in Dict_all:
            if key in art:
                increase_keyword_dict[key][5] += 1

    for key in decrease_keyword_dict.keys():
        decrease_keyword_dict[key].append(0)
        for art in Dict_all:
            if key in art:
                decrease_keyword_dict[key][5] += 1

    for key in increase_keyword_dict.keys():
        if increase_keyword_dict[key][1] - ((increase_keyword_dict[key][5]/num_all) * num_doc) >= 0:
            increase_keyword_dict[key].append(((increase_keyword_dict[key][1] - ((increase_keyword_dict[key][5]/num_all) * num_doc)) ** 2)/((increase_keyword_dict[key][5]/num_all) * num_doc))
        else:
            increase_keyword_dict[key].append((((increase_keyword_dict[key][1] - ((increase_keyword_dict[key][5]/num_all) * num_doc)) ** 2)/((increase_keyword_dict[key][5]/num_all) * num_doc))* -1)

    increase_keyword_dict = sorted(increase_keyword_dict.items(),key = lambda x:x[1][6], reverse = True)  #照DF卡方排

    for key in decrease_keyword_dict.keys():
        if decrease_keyword_dict[key][1] - ((decrease_keyword_dict[key][5]/num_all) * num_doc) >= 0:
            decrease_keyword_dict[key].append(((decrease_keyword_dict[key][1] - ((decrease_keyword_dict[key][5]/num_all) * num_doc)) ** 2)/((decrease_keyword_dict[key][5]/num_all) * num_doc))
        else:
            decrease_keyword_dict[key].append((((decrease_keyword_dict[key][1] - ((decrease_keyword_dict[key][5]/num_all) * num_doc)) ** 2)/((decrease_keyword_dict[key][5]/num_all) * num_doc))* -1)
    decrease_keyword_dict = sorted(decrease_keyword_dict.items(),key = lambda x:x[1][6], reverse = True)  #照DF卡方排
    ...
    for inc in increase_keyword_dict:
        if inc[1][1] > dfthresh and inc[1][6] > chithresh:
            increase_keyword.append(inc)
    for dec in decrease_keyword_dict:
        if dec[1][1] > dfthresh and dec[1][6] > chithresh:
    ...
    np.save('./0504/output_X_'+ver+'_ddc'+str(dfthresh)+str(chithresh)+'.npy', output_x_numpy)
    np.save('./0504/output_Y_'+ver+'_ddc'+str(dfthresh)+str(chithresh)+'.npy', output_y_numpy)
